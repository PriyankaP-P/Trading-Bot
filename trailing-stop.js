...
async function start_trailing(){
    try
    {
        ...
        let new_longs = await database('transactions').where({ transaction_type: 'buy',
                                                                fulfilled: 'true',
                                                                order_status: 'closed',
                                                                position_status: 'new',
                                                                })
        ...
        for(let i =0; i<new_longs.length ; i++){
            ...
            let open_trailing_positions = await database('trail').where('trailing_status', 'true')
            ...
            if(open_trailing_positions.length >0){
                for(let j=0; j< open_trailing_positions; j++){
                    if(new_longs[i].transaction_id !== open_trailing_positions[j].transaction_id){

                        let price = await stoploss.get_price(symbol_pair);
                        await database('trail').insert({ transaction_id: transac_id,
...
async function update_highest_price(){
    try{
        let open_trail = await database('trail').where('trailing_status', 'true')
                                                .select()
                                                .then(row => row)
...
async function trailing_stop_func(trailing_percent){
    try{
        ...
        for(let i =0; i<check_price.length; i++){
            ...
            let price_condition = check_price[i].trailing_price -(check_price[i].trailing_price * percent_conv);
            ...
            if(open_sell_orders_for_trailing.length >0){        
                ...
                for(let j =0; j< open_sell_orders_for_trailing.length; j++){
                        ...
                        if(to_check === true  && 
                            check_price[i].transaction_id !== open_sell_orders_for_trailing[j].selling_pair_id){

                            let get_buy_pair_data = await database('transactions').where('transaction_id', check_price[i].transaction_id)
                                                                                  .select()
                                                                                  .then(row => row)
                                                                                  .catch(e => console.log(e))
            
                            await database('transactions').insert({ trade_date: date, symbol_pair: check_price[i].symbol_pair,
                                price_base_currency: currentPrice,
                                equivalent_amt_base_currency: get_buy_pair_data[0].equivalent_amt_base_currency,
                                transaction_type: 'sell', fulfilled: 'f',
                                order_status: 'open', selling_pair_id: get_buy_pair_data[0].transaction_id})
                                .then(rows => rows)
                                .catch(error =>console.log(error))
