...
async function cut_loss(stop_loss_percent) {
  try {
    ...
    for (let i = 0; i < current_buys.length; i++) {
      ...
      let open_sell_orders_for_stoploss = await database("transactions") //from here till next comment mark code repeated in trailing-stop , convert to function
        .where({
          ...
          if (
            ...
            await database("transactions")
              .insert({
                ...
                equivalent_amt_base_currency:
                  current_buys[i].equivalent_amt_base_currency,
                transaction_type: "sell",
                fulfilled: "f",
                order_status: "open",
